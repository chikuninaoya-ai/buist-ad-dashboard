...
def _meta_get(path, params):
    cfg = st.secrets["meta"]
    gv = cfg.get("graph_version", "v25.0")
    p = {**params, "access_token": cfg["access_token"]}
    url = f"https://graph.facebook.com/{gv}/{path}?" + urllib.parse.urlencode(p)
    rows = []
    while url:
...
def fetch_meta_daily_spend(lookback_days=210):
    """キャンペーン別・日別の消化金額。CR別予算シートと同じ列(Day, Campaign Name, Amount Spent)を返す。"""
    cfg = st.secrets["meta"]
    today = pd.Timestamp.now(tz="Asia/Tokyo").normalize().tz_localize(None)
    since = (today - pd.Timedelta(days=lookback_days)).strftime("%Y-%m-%d")
    ...
    data = [{"Day": x["date_start"], "Campaign Name": x.get("campaign_name", ""),
             "Amount Spent": str(x.get("spend", "0"))} for x in rows]
    if not data:
        return pd.DataFrame(columns=["Day", "Campaign Name", "Amount Spent"])
    return pd.DataFrame(data)


@st.cache_data(ttl=600, show_spinner=False)
def fetch_meta_campaign_status():
    """キャンペーンの配信状態(ACTIVE/PAUSED等)。"""
    cfg = st.secrets["meta"]
    rows = _meta_get(f"{cfg['ad_account_id']}/campaigns",
                     {"fields": "name,effective_status,daily_budget", "limit": "500"})
    return pd.DataFrame([{"Campaign Name": x.get("name", ""),
                          "配信状態": x.get("effective_status", ""),
                          "日予算": int(x.get("daily_budget", 0) or 0)} for x in rows])
...
def load_data(target_year_month="2026-03"):
    ...
    # ── 1. 消化予算（Meta Graph API からライブ取得） ──
    df_budget = fetch_meta_daily_spend()
    if not all_mode:
        df_budget = df_budget[df_budget["Day"].astype(str).str.startswith(target_year_month)]
...
def render_campaign_status():
    st.subheader("📡 キャンペーン稼働状況（Meta・リアルタイム）")
    try:
        status_df = fetch_meta_campaign_status()
        spend_df = fetch_meta_daily_spend()
    except Exception as e:
        st.error(f"Meta APIの取得に失敗: {e}")
        return
    if status_df.empty:
        st.info("キャンペーンなし")
        return
    sp = spend_df.copy()
    if not sp.empty:
        sp["Amount Spent"] = sp["Amount Spent"].str.replace(",", "").astype(float)
        sp = sp.groupby("Campaign Name")["Amount Spent"].sum().reset_index()
    else:
        sp = pd.DataFrame(columns=["Campaign Name", "Amount Spent"])
    m = status_df.merge(sp, on="Campaign Name", how="left").fillna({"Amount Spent": 0})
    LABEL = {"ACTIVE": "🟢 稼働中", "PAUSED": "⏸ 停止", "CAMPAIGN_PAUSED": "⏸ 停止",
             "ADSET_PAUSED": "⏸ 停止(広告セット)", "IN_PROCESS": "🟡 処理中",
             "WITH_ISSUES": "⚠️ 問題あり", "DISAPPROVED": "❌ 非承認", "ARCHIVED": "📦 アーカイブ"}
    m["状態"] = m["配信状態"].map(lambda s: LABEL.get(s, s))
    m = m.sort_values("Amount Spent", ascending=False)
    n_active = int((status_df["配信状態"] == "ACTIVE").sum())
    c = st.columns(3)
    c[0].metric("総キャンペーン", len(status_df))
    c[1].metric("🟢 稼働中", n_active)
    c[2].metric("⏸ 停止中", len(status_df) - n_active)
    m_disp = m[["状態", "Campaign Name", "Amount Spent", "日予算"]].copy()
    m_disp["Amount Spent"] = m_disp["Amount Spent"].apply(lambda v: format_yen(int(v)))
    m_disp["日予算"] = m_disp["日予算"].apply(lambda v: format_yen(int(v/100)) if v else "—")
    m_disp.columns = ["状態", "キャンペーン名", "期間消化", "日予算"]
    st.dataframe(m_disp, hide_index=True, use_container_width=True)


def main():
    st.title("📈 ブイスト広告ダッシュボード")
    st.caption("Meta Graph API ライブ連携 ＋ Google Sheets｜消化予算・キャンペーン稼働状況は自動更新")
    _ad_dashboard()
    st.divider()
